    ...
    <div className="flex flex-col gap-6">
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((item) => {
            const stockQty = item.inventoryBins.reduce((sum: number, inv) => sum + inv.quantity, 0);
            const margin = Number(item.salePrice) - Number(item.bulkPrice);
              ...
              <div key={item.id} className="neu-card p-5 flex flex-col justify-between gap-4">
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-neu-pressed text-neu-accent">
                      {locale === "ur-PK" ? item.category.nameUr : item.category.nameEn}
                    </span>
                ...
                <div className="bg-neu-pressed p-3 rounded-xl flex flex-col gap-1.5 shadow-neu-inset">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-neu-text">PKR {Number(item.bulkPrice).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-neu-muted">Recommended Retail:</span>
                    <span className="font-semibold text-neu-muted">PKR {Number(item.salePrice).toLocaleString()}</span>
                  </div>
